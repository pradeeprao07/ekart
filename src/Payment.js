    ...
    export default function Payment() {
        ...
        const location = useLocation(); // useLocation is used to receive the data being sent from useNavigate
        const totalOfAllItem = location.state.totalOfAllItem || 0; // total price being received from the cart
        const navigate = useNavigate();

        const handleCardNumber = (ip) => {
            let newBankValue = ip.target.value.replace(/[^0-9]/g, "").slice(0, 16) // replace lets only to remain the numbers and slice of 16 elements 
            newBankValue = newBankValue.replace(/(\d{4})/g, "$1 ").trim() // ensures to have a space after every 4 ip
            setCardNo(newBankValue)
        }

        const handleValidTill = (ip) => {
            let newValidTillValue = ip.target.value.replace(/[^0-9]/g, ""); 
            if (newValidTillValue.length > 4) return; // if the value exceeds more than 4 ip it should exit(return)
            if (newValidTillValue.length >= 2) {
                newValidTillValue = newValidTillValue.slice(0, 2) + "/" + newValidTillValue.slice(2); // if >= 2 a '/' in b/w
            }
            setValidTillValue(newValidTillValue);
        }

        const handleCvv = (e) => {
            let newCvvValue = e.target.value.replace(/[^0-9]/g, "").slice(0, 3);
            setCvvValue(newCvvValue)
        }

        const handlePayButton = () => {
            if (!cardNo && !selectUpi && !selectBank) {
